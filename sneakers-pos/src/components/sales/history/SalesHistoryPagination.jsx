...
export default function SalesHistoryPagination({
  page = 1, perPage = 25, total = 0, onPageChange, onPerPageChange,
}) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-t border-gray-100 dark:border-dark-border">
      <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-dark-muted">
        <span>
          Mostrando <span className="font-medium text-brand-black dark:text-dark-text">{start}</span>–<span className="font-medium text-brand-black dark:text-dark-text">{end}</span> de <span className="font-medium text-brand-black dark:text-dark-text">{total}</span> ventas
        </span>
