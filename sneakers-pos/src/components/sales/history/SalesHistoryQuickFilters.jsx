const FILTERS = [
  ...
  { key: 'partial_return',  label: 'Parcialmente devueltas' },
  { key: 'returned',        label: 'Devueltas' },
  { key: 'cancelled',       label: 'Canceladas' },
]

export default function SalesHistoryQuickFilters({ active = 'all', counts = {}, onChange }) {
  return (
    <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-1 -mx-1 px-1">
      {FILTERS.map(({ key, label }) => {
        const isActive = active === key
        const count = counts[key]
        const isDestructive = ['cancelled', 'returned'].includes(key)
        return (
          <button
            key={key}
            type="button"
